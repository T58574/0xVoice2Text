...
def run_system_verification():
    ...
    # 5. TTS Subsystem & Echo Filtering
    tts = JarvisVoiceService(config)
    print(f"[+] JarvisVoiceService initialized -> Active: {tts.adapter.get_name()}")
    echo_detected = tts.is_jarvis_phrase("Слушаю вас, сэр.")
    print(f"[+] TTS Anti-Self-Echo filter check: {echo_detected} (Expected: True)")
    assert echo_detected == True, "Anti-echo check failed!"
